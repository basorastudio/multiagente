...
const AdminCreateTenantService = async ({
  ...
  const tenant = await Tenant.create({
    ...
    messageBusinessHours: "¡Hola! ¡Fantástico recibir tu contacto! En este momento no estamos disponibles y no podremos atenderte, pero priorizaremos tu solicitud y te responderemos lo antes posible. Agradecemos mucho tu contacto."
  });

  // Insertar configuraciones por defecto para el nuevo inquilino
  const defaultSettings = [
    ...
    { key: 'callRejectMessage', value: 'Las llamadas de voz y video están deshabilitadas para este WhatsApp, por favor envía un mensaje de texto.' }
  ];
