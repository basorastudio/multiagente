...
const _htmlEscape = string =>
  string
    .replace(/&/g, "&amp;") // Debe ocurrir primero o de lo contrario escapará otros caracteres recién escapados.
    .replace(/"/g, "&quot;")
    .replace(/'/g, "&#39;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");
...
const _htmlUnescape = htmlString =>
  htmlString
    ...
    .replace(/&#0?39;/g, "'")
    .replace(/&quot;/g, '"')
    .replace(/&amp;/g, "&"); // Debe ocurrir al final o de lo contrario desescapará otros caracteres en el orden incorrecto.
...
export const pupa = function pupa(
  ...
  if (typeof template !== "string") {
    throw new TypeError(
      `Se esperaba un \`string\` en el primer argumento, se obtuvo \`${typeof template}\``
    );
  }
  ...
  // La expresión regular intenta coincidir con un número dentro de `{{ }}` o con un identificador JS válido o una ruta de clave.
