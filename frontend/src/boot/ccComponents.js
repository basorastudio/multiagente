...
const iniciaisString = nombrecompleto => {
  nombrecompleto = nombrecompleto.replace(/\s(de|da|dos|das)\s/g, ' ') // Elimina los de, da, dos, das.
  const iniciales = nombrecompleto.match(/\b(\w)/gi) // Iniciales de cada parte del nombre.
  // var nombre = nombrecompleto.split(' ')[0].toLowerCase() // Primer nombre.
  const apellidos = iniciales
    .splice(1, iniciales.length - 1)
    .join('')
    .toLowerCase() // Iniciales
  const inicialesNombre = iniciales + apellidos
  return inicialesNombre.toUpperCase()
}
...
const setConfigsUsuario = ({ isDark }) => {
  ...
  const filtrosAtendimento = JSON.parse(localStorage.getItem('filtrosAtendimento')) || filtroPredeterminado
  const data = {
    filtrosAtendimento,
    isDark: Dark.isActive
  }
  UpdateConfiguracoesUsuarios(usuario.userId, data)
    .then(r => console.log('Configuraciones del usuario actualizadas'))
