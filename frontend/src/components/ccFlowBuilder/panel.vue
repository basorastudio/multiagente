  ...
  methods: {
    ...
    jsPlumpConsist (evt) {
      ...
      if (from === to) {
        this.$q.notify({
          ...
          position: 'top',
          timeout: 2500,
          message: 'No es posible conectar el elemento a sí mismo.',
          actions: [{
            icon: 'close',
            round: true,
            color: 'white'
      ...
      for (var i = 0; i < this.data.nodeList.length; i++) {
        ...
        if (!node.viewOnly) {
          this.jsPlumb.draggable(node.id, {
            containment: 'parent',
            stop: function (el) {
              console.log('arrastrado al final: ', el)
            }
          })
        }
      }
    ...
    deleteElement () {
      if (this.activeElement.type === 'node') {
        ...
        this.$q.dialog({
          title: '¡Atención!',
          message: '¿Desea eliminar realmente la línea seleccionada?',
          cancel: {
            label: 'No',
            color: 'primary',
            push: true
    ...
    deleteNode (node) {
      this.$q.dialog({
        title: '¡Atención!',
        message: `¿Desea eliminar realmente el elemento (${node.name})?`,
        cancel: {
          label: 'No',
          color: 'primary',
          push: true
