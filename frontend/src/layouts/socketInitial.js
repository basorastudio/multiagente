  ...
  methods: {
    socketInitial () {
      ...
      socket.on(`${usuario.tenantId}:ticketList`, async data => {
        console.log('socket ENCENDIDO')
        if (data.type === 'chat:create') {
          console.log('chat:crear')
      ...
      socket.on(`${usuario.tenantId}:change_battery`, data => {
        this.$q.notify({
          message: `Batería del celular de WhatsApp ${data.batteryInfo.sessionName} está en ${data.batteryInfo.battery}%. Es necesario iniciar carga.`,
          type: 'negative',
          progress: true,
          position: 'top',
          actions: [{
