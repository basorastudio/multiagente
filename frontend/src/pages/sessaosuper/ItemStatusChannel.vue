    ...
    <q-item>
      ...
      <q-item-section>
        <q-item-label v-if="item.status == 'qrcode'">
          ...
          <span class="row col"> Solicite a su cliente que lea el QR Code </span>
        </q-item-label>
        ...
        <q-item-label v-if="item.status == 'DISCONNECTED'">
          ...
          > Asegúrese de que su celular esté conectado a internet e intente nuevamente, o solicite un nuevo QR Code </span>
          <span
            class="row col"
            v-if="item.type === 'telegram'"
          > Intente conectar nuevamente. Si el error persiste, confirme que el token sea correcto.</span>
          <span
            class="row col"
            v-if="item.type === 'instagram'"
          > Intente conectar nuevamente. Si el error persiste, confirme que las credenciales sean correctas.</span>
        </q-item-label>
        ...
        <q-item-label v-if="['PAIRING', 'TIMEOUT'].includes(item.status)">
          ...
          <span class="row col"> Asegúrese de que su celular esté conectado a internet y WhatsApp esté abierto, o haga clic en el botón 'Desconectar' para obtener un nuevo QR Code </span>
        </q-item-label>
