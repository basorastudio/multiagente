  ...
  methods: {
    ...
    async consultarTickets (paramsInit = {}) {
      ...
      try {
        ...
        this.countTickets = data.count // cuenta total de tickets en el estado
    ...
    ticketListSocket () {
      ...
      const notBelongsToUserQueues = (ticket) =>
        ticket.queueId && this.queuesIds.indexOf(ticket.queueId) === -1 // No pertenece a las colas del usuario
      ...
      this.socket.on(`tenant:${usuario.tenantId}:appMessage`, (data) => {
        if (data.action === 'create' && shouldUpdateTicket(data.ticket)) {
          ...
          // Si el ticket no está enfocado, no está cerrado, el mensaje no es mío y no es de un flujo de chat
